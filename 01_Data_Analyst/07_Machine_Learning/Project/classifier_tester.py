...
def classifier_test(clf, dataset, feature_list, splits=1000):
    data = featureFormat(dataset, feature_list, sort_keys=True)
    labels, features = targetFeatureSplit(data)
         
    cv = StratifiedShuffleSplit(n_splits=splits, test_size=0.3, train_size=0.7, random_state=42)
    ...
    for train_idx, test_idx in cv.split(features, labels):
        ...
        for prediction, truth in list(zip(predictions, labels_test)):
...
def main():
    ...
    # Run testing script
    classifier_test(clf, dataset, feature_list)
